...
class NGramModel:
    ...
    def __init__(self, n: int = 2):
        ...
        self.n = n
        self.ngram_counts: Dict[int, Dict[Tuple[str, ...], int]] = defaultdict(
            lambda: defaultdict(int)
        )
        self.context_counts: Dict[int, Dict[Tuple[str, ...], int]] = defaultdict(
            lambda: defaultdict(int)
        )
    ...
    def train(
        self, corpus: List[List[str]], smoothing: str = "laplace", alpha: float = 1.0
    ):
    ...
    def _apply_smoothing(
        self, ngram_count: int, context_count: int, vocab_size: int
    ) -> float:
        if self._smoothing == "laplace":
            return (ngram_count + self._alpha) / (
                context_count + self._alpha * vocab_size
            )
...
class BigramModel(NGramModel):
    ...
    def extract_collocations(
        self, min_freq: int = 2, min_pmi: float = 0.0, top_k: int = 20
    ...
    def get_frequent_bigrams(
        self, min_freq: int = 2, top_k: int = 20
    ...
    def get_word_collocations(
        self, word: str, position: str = "after", min_freq: int = 1, top_k: int = 10
...
__all__ = ["BigramModel", "NGramModel", "TrigramModel"]
