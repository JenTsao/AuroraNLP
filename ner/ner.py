...
class NERFeatureTemplate:
    ...
    def add_place_suffix_feature(self, position: int):
        PLACE_SUFFIXES = set(
            "省市县区镇乡村路街道州岛山江河湖海港湾城关省市区县镇乡村路街道"
        )
...
class CRFNERModel:
    def __init__(
        self, tags: Optional[List[str]] = None, entity_types: Optional[Set[str]] = None
    ):
    ...
    def train(
        ...
        for chars, tags in corpus:
            if len(chars) != len(tags):
                raise ValueError(
                    f"Chars and tags length mismatch: {len(chars)} vs {len(tags)}"
                )
    ...
    def _forward(
        self, chars: List[str], seq_features: List[Dict[str, List[str]]]
    ) -> List[Dict[str, float]]:
        length = len(chars)
    ...
    def _extract_entities(self, chars: List[str], tags: List[str]) -> List[Entity]:
        ...
        while i < len(tags):
            ...
            if tag.startswith("S-"):
                entity_type = tag[2:]
                entities.append(
                    Entity(text=chars[i], entity_type=entity_type, start=i, end=i + 1)
                )
                i += 1
...
def train_ner_from_file(
    recognizer: NERRecognizer, filepath: str, encoding: str = "utf-8"
) -> None:
    corpus = []
...
class NestedNERRecognizer:
        ...
        for level in range(1, max_levels):
            ...
            for parent_entity in entities_at_level:
                ...
                for inner in inner_entities:
                    if (
                        inner.text == parent_entity.text
                        and inner.entity_type == parent_entity.entity_type
                    ):
                        continue
    ...
    def get_entities_by_level(
        self, text: str, level: int, max_levels: int = 3
    ) -> List[NestedEntity]:
        hierarchy = self.recognize_nested(text, max_levels)
        ...
        for entity in entities:
            if entity.start >= last_end:
                ...
                if entity.is_nested():
                    ...
                    annotation = (
                        f"[{entity.text}/{entity.entity_type}(L{entity.level})]"
                    )
